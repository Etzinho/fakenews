...
function bp_dtheme_enqueue_scripts() {
	...
	wp_enqueue_script( 'api-js', get_template_directory_uri() . '/_inc/api.js', array( 'jquery') );
...
function bp_dtheme_enqueue_styles() {
	...
	wp_register_style( 'custom_fakenews', get_template_directory_uri() . '/_inc/css/custom.css', array(), bp_get_version() );
	...
	wp_enqueue_style( 'custom_fakenews' );
...
function bp_dtheme_blog_comments( $comment, $args, $depth ) {
	...
	$vote = "Não votou";
	$class_vote = "";
	if(is_array($truth) && in_array($comment->user_id, $truth)){
		$class_vote = "vote-truth";
		$vote = "Votou Verdadeiro";	
	} 
	$false = get_post_meta($post->ID, "false", TRUE);
	if(is_array($false) && in_array($comment->user_id, $false)){
		$class_vote = "vote-false";
		$vote = "Votou Falso";
	} 
	?>

	<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
		<div class="comment-avatar-box" style="position: relative;">
			<div class="avb">
			...
			<div class="vote <?= $class_vote ?>" style="position: absolute; top:100%; left: 0; right: 0; text-align: center;">
				<?= $vote ?>
			</div>
		</div>
...
require get_template_directory() . '/api/routes.php';
